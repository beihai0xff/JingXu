...
- [ ] 当前图库初始化与重开、完整 v4 旧库备份后转换、未知旧格式拒绝且内容不变、WAL 备份、锁与恢复中断检查通过。
- [ ] 受限目录、空间不足及真实进程中断下，停止写入且备份可恢复。
- [ ] 独立测试环境完成 v4 副本逐字段比较、未知格式拒绝、原库及日志保留，以及新格式图库的目录授权、保存、重启与核对。
...
- [ ] 调色滑块与数字输入焦点、原图对比、自动保存、失败重试、切图／关闭／退出保存、100% 与缩放保持、预设与批量确认、成片导出并重新打开通过。
