# Developer ID 签名、公证与 Homebrew 发布
...
1. 更新 `Packaging/Info.plist` 的版本号和递增构建号，新增对应 `Documentation/Release-<版本>.md`，同步需要变动的安装说明。首次正式签名发布应使用新版本，不能覆盖既有 0.2.6 安装包。
2. 按 [发布检查清单](../Packaging/ReleaseChecklist.md) 验证本次提交，尤其是从临时签名版升级、原容器访问及照片目录授权。
3. 提交并合入 `main`，等待构建通过。所有签名配置就绪后，在干净且与 `origin/main` 一致的检出上执行：

   ```sh
   export RELEASE_ACCEPTANCE_COMMIT="已完成验收的完整提交 SHA"
...
- Cask 只接受正式数字版本，拒绝草稿、预发布、倒退版本、同版本摘要变化、缺失摘要和错误下载地址。同版本同摘要重复执行不写入。
