...
/usr/bin/security find-identity -v -p codesigning | /usr/bin/grep -F -- "$DEVELOPER_ID_APPLICATION" >/dev/null || { print -u2 "找不到有效签名身份。"; exit 1; }
...
xattr -cr "$APP_DIR"
codesign --force --options runtime --timestamp --sign "$DEVELOPER_ID_APPLICATION" --entitlements JingXu.entitlements "$APP_DIR"
...
xcrun notarytool submit "${PACKAGE_TEMP}/notary-upload.zip" --keychain-profile "$NOTARY_KEYCHAIN_PROFILE" --wait --output-format plist > "${PACKAGE_TEMP}/app-notary.plist"
...
codesign --timestamp --sign "$DEVELOPER_ID_APPLICATION" "${PACKAGE_TEMP}/release.dmg"
xcrun notarytool submit "${PACKAGE_TEMP}/release.dmg" --keychain-profile "$NOTARY_KEYCHAIN_PROFILE" --wait --output-format plist > "${PACKAGE_TEMP}/dmg-notary.plist"
