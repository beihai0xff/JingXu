...
[[ "${RELEASE_ACCEPTANCE_COMMIT:-}" == "$COMMIT" ]] || { print -u2 "按发布通道完成检查并记录未验收项后，将 RELEASE_ACCEPTANCE_COMMIT 设置为该提交完整 SHA。"; exit 1; }
...
print "已推送 ${TAG}。GitHub Actions 将按工作流的显式发布通道执行构建、签名、Release 和 Cask 更新。"
